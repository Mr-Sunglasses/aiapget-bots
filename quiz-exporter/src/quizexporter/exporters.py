...
def _find_template_dir() -> Path:
    """Locate the templates directory. Checks multiple locations."""
    candidates = [
        # When running from quiz-exporter/ dir: ../templates
        Path(__file__).resolve().parent.parent.parent.parent / "templates",
        # When running in Docker: /app/templates
        Path("/app/templates"),
        # Relative to CWD
        Path("templates"),
    ]
...
def _parse_quiz_json(data: list | dict) -> tuple[str, str, list[dict]]:
    """Return (file_name, title, list_of_polls) from the quiz JSON.
...
def get_file_name(data: list | dict) -> str:
    """Extract file_name from the quiz JSON, or return empty string."""
...
def _right_answer_number(correct_option_id: int | None) -> int | str:
    """Return 1-based numeric answer for XLSX (A=1, B=2, C=3, D=4, ...)."""
    if correct_option_id is None:
        return ""
...
# Header styling (blue background, white bold text, centred)
...
def generate_xlsx(data: list | dict, output_path: Path) -> Path:
    ...
    # Try to use the template; fall back to creating from scratch.
    if XLSX_TEMPLATE.exists():
        wb = openpyxl.load_workbook(XLSX_TEMPLATE)
        ws = wb.active
        # Delete any existing sample data rows (keep header)
        if ws.max_row > 1:
...
# ---------------------------------------------------------------------------
# DOCX export  (matches template format from question_output.docx)
# ---------------------------------------------------------------------------
#
# Template structure per question:
#
#   ###Section AYURVEDA           <- section header (first line only)
#   (empty line)
#    #English_directions
#    #Question N
#   (empty line)
...
#   #tag
#   <title>
#   (empty line)                  <- separator before next question


# Maximum option slots in the template (A..E)
_MAX_DOCX_OPTIONS = 5
...
def generate_docx(data: list | dict, output_path: Path) -> Path:
    ...
    for idx, poll in enumerate(polls, start=1):
        ...
        for opt_idx in range(_MAX_DOCX_OPTIONS):
            ...
            if opt_idx == 0:
                # First option line includes #Options marker
        ...
        doc.add_paragraph(explanation if explanation else "As per reference in the original question.")
